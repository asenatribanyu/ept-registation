...
class Event_modeljp extends CI_Model
{
    ...
    public function get($id_event_jp = null)
    {
    ...
    { # kondisi = array();
        $this->db->select('tejp.id_event_jp, DATE_FORMAT(tejp.tanggal_event, "%e %M %Y") AS tanggal_event, tejp.type, tejp.time, tejp.venue, COUNT(trjp.id_event_jp) AS total, tejp.kuota - COUNT(trjp.id_event_jp) AS sisa_kuota, tejp.aktif');
        $this->db->from('tbl_event_jp AS tejp');
        $this->db->join('tbl_registrant_jp as trjp', 'tejp.id_event_jp = trjp.id_event_jp', 'LEFT');
